...
public class SceneUnit{
  ...
  public string GetNameAndPath(){
	...
	public List<string> GetSceneName()
	{
		...
		foreach (string path in _pathList)
		{
			sceneName.Add(Path.GetFileNameWithoutExtension(path));
		}
		return sceneName;
	}
}
...
public class SceneUnitSet{
  ...
  public void Move(SceneUnit sceneUnit, bool isUp){
	...
	public void SaveSceneListTable()
	{
		var _sceneListTable = AssetDatabase.LoadAssetAtPath<SceneListTable>("Assets/SceneListTable.asset");
		if(_sceneListTable == null)
		{
			_sceneListTable = ScriptableObject.CreateInstance<SceneListTable>();
			AssetDatabase.CreateAsset(_sceneListTable, "Assets/SceneListTable.asset");
